...
static const uint32_t four_p_limbs[13] = {
  0x7FFEAAAC, // limb 0  = bits [0..30]
  0x4FF7FFFF, // limb 1  = bits [31..61]
  0x153FFFFB, // limb 2  = bits [62..92]
  0x557FFFD6, // limb 3  = bits [93..123]
  0x2C3D8907, // limb 4  = bits [124..154]
  0x1869507B, // limb 5  = bits [155..185]
  0x0512BF67, // limb 6  = bits [186..216]
  0x6E9709E7, // limb 7  = bits [217..247]
  0x2EB35D91, // limb 8  = bits [248..278]
  0x5D3DB21A, // limb 9  = bits [279..309]
  0x7E69A4B1, // limb 10 = bits [310..340]
  0x223D472F, // limb 11 = bits [341..371]
  0x00000680, // limb 12 = bits [372..402]
};
...
void ksub13_4N(
  ...
  v8int32 four_p_limbs_vector[13];
  for (int i = 0; i < 13; i++) {
    for (int j = 0; j < 8; j++) {
      four_p_limbs_vector[i] = upd_elem(four_p_limbs_vector[i], j, four_p_limbs[i]);
    }
  }
  v8int32 all_1;
  for (int i = 0; i < 13; i++) {
  ...
  acc = mac(acc, four_p_limbs_vector[0], all_1);
  ...
  for (int i = 1; i < 12; i++) {
    ...
    acc = mac(acc, four_p_limbs_vector[i], all_1);
  ...
  acc = mac(acc, four_p_limbs_vector[12], all_1);
  acc = mac(acc, - all_1, all_1);
